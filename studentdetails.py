...
class Student:
    def __init__(self, root):
        ...
        course_frame = LabelFrame(Left_frame, bd=2, bg="white",
                                  relief=RIDGE, text="Course", font=("arial", 15, "bold"))
        ...
        dep_combobox = ttk.Combobox(course_frame, font=(
            "arial", 12, "bold"), textvariable=self.dep_var, state="readonly")
        dep_combobox["values"] = ("Select Department", "DoCSE", "DoEE",
                                  "Mechanical", "Environmental", "Pharmacy", "Architecture")
        dep_combobox.grid(row=0, column=1, padx=2, pady=10)

        year_label = Label(course_frame, text="Year",
                           font=("arial", 12, "bold"), bg="white")
        ...
        year_combobox = ttk.Combobox(course_frame, font=(
            "arial", 12, "bold"), textvariable=self.year_var, state="readonly")
        ...
        def on_department_change(*args):
            ...
            if selected_department == "Select Department" or selected_year == "Select Year":
                ...
                selected_options = course_options.get(
                    (selected_department, selected_year), ("Select Course",))
        ...
        course_combobox = ttk.Combobox(course_frame, textvariable=self.var_course, font=(
        ...
        # for name
        name_label = Label(student_frame, text="Name",
                           font=("arial", 12, "bold"), bg="white")
        name_label.grid(row=0, column=0, padx=10, sticky=W)

        name_entry = ttk.Entry(
            student_frame, textvariable=self.var_name, width=20, font=("arial", 12, "bold"))
        name_entry.grid(row=0, column=1, padx=10, sticky=W)

        # for roll no
        roll_label = Label(student_frame, text="Roll no",
                           font=("arial", 12, "bold"), bg="white")
        roll_label.grid(row=0, column=2, padx=10, sticky=W)

        roll_entry = ttk.Entry(
            student_frame, textvariable=self.var_rollno, width=5, font=("arial", 12, "bold"))
        roll_entry.grid(row=0, column=3, padx=10, sticky=W)

        # for sid
        sid_label = Label(student_frame, text="Id", font=(
            "arial", 12, "bold"), bg="white")
        sid_label.grid(row=0, column=4, padx=10, sticky=W)

        sid_entry = ttk.Entry(
            student_frame, textvariable=self.var_sid, width=10, font=("arial", 12, "bold"))
        sid_entry.grid(row=0, column=5, padx=10, sticky=W)

        # photo sample
        self.var_rad1 = StringVar()
        radio_btn1 = ttk.Radiobutton(
            student_frame, text="Take photo sample", variable=self.var_rad1, value="Yes")
        radio_btn1.grid(row=2, column=0)

        radio_btn2 = ttk.Radiobutton(
            student_frame, text="No photo sample", variable=self.var_rad1, value="No")
        ...
        # save
        save = Button(btn_frame, command=self.add, text="Save", width=15, font=(
            "arial", 12, "bold"), bg="green", fg="white",)
        save.grid(row=1, column=1, padx=2)
        # update
        update = Button(btn_frame, command=self.update, text="Update", width=15, font=(
            "arial", 12, "bold"), bg="blue", fg="white")
        update.grid(row=1, column=3, padx=2)
        # delete,
        delete = Button(btn_frame, command=self.delete, text="Delete", width=15, font=(
        ...
        search_frame = LabelFrame(
            Right_frame, bg="white", relief=RIDGE, text="Search", font=("arial", 15, "bold"))
        ...
        self.table = ttk.Treeview(table_frame, column=("dep", "course", "year", "name",
                                  "roll no", "photo"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
            ...
            try:
                ...
                if update > 0:
                    ...
                    if not update:
                        return
                messagebox.showinfo(
                    "Success", "Update successfully", parent=self.root)
        ...
        if self.var_rollno == "":
            ...
            try:
                ...
                if delete > 0:
                    ...
                    my_cursor = conn.cursor()
                    my_cursor.execute(
                        "delete  from student where sid =%s", (self.var_sid.get(),))
                else:
                    if not delete:
                        return
                conn.commit()
                ...
                conn.close()
                messagebox.showinfo(
                    "Delete", "Deleted succesfully", parent=self.root)
        ...
        if self.var_dep.get() == "Select Department" or self.var_name.get() == "":
            ...
            try:
                ...
                def face_crop(img):
                    ...
                    # scaling factor=1.3 min neighborhood=5
                ...
                while True:
                    ...
                    if face_crop(my_frame) is not None:
                        ...
                        file_path = "data/user." + \
                            str(self.var_sid.get())+"."+str(img_id)+".jpg"

                        cv2.imwrite(file_path, face)
                        # font size during cam
